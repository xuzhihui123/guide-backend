...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getAllQues() {
        let data = await getAllQuestion();
        data.forEach(item => {
          let dateTime = item.gmtCreate;
          let d = parseInt(this.formatTime(dateTime));
          switch (d) {
            case 1:
      ...
      async getAllOrdersArray(){
        let d= await  getAllAlreadyOrders()
        let data = d.msg
        data.forEach(item=>{
          let ds = this.formatTime(parseInt(item.orderTime))
          let d = parseInt(ds)
          switch (d) {
            case 1:
